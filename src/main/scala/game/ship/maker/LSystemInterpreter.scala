...
object LSystemInterpreter {
  ...
  case class DefaultInterpreter[T](
                                    forward: T,
                                    left: T,
                                    right: T,
                                    incWidth: T,
    ...
    val mappings: (T, State[T]) => (Seq[Command], State[T]) = (t, state) =>
      t match
        case `forward` =>
    ...
    def forwardCommands(state: State[T]): (Seq[Command], State[T]) =
      ...
      (
        Seq(
          ...
          Command.OffsetPosition(positionDelta), //todo mb OffsetLocal(V2(forwardLength, 0))
        ),
